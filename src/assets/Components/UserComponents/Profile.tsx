...
export default function LeetCodeProfile() {
  ...
  const centerTextPlugin: Plugin = {
    ...
    beforeDraw(chart: any) {
      ...
      ctx.save();
      ctx.font = `${fontSize}px sans-serif`; // Fixed interpolation error here
  ...
  const toggleEditMode = () => {
    if (isEditing) {
      setProfileData(editData); // Save changes
    }
    setIsEditing(!isEditing); // Toggle editing state
  };

  // Filter questions based on selected language and level
  const filteredQuestions = questions.filter((question) => {
    let matchesLanguage = selectedLanguage === 'All' || question.language === selectedLanguage;
    let matchesLevel = selectedLevel === question.level;
